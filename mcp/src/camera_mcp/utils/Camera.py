...
class Camera(object):
    ...
    def img_catch(self, return_dict, img_name):
        ...
        # 使用项目根目录的 img 文件夹
        img_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), 'img', date_img)
        ...
        if success:
            cv2.imwrite(os.path.join(img_dir, return_dict['img_name']), img)
...
if __name__ == '__main__':
    C1 = Camera('192.168.1.64', 'admin', 'pw4hkcamera')
    C1.catch()